...
//   // Reorder videotableData based on drag result
//   const reorderedData = Array.from(videotableData);
//   const [removed] = reorderedData.splice(result.source.index, 1);
//   reorderedData.splice(result.destination.index, 0, removed);

//   // Update state with reordered video table data
//   setvideoTableData(reorderedData);

//   // Map through reorderedData to get video titles based on videoId
//   const reorderedTitles = reorderedData.map(item => {
//     const titleObject = videotitle.find(cat => cat.id === item.videoId);
//     return titleObject ? titleObject.videoTitle : "";  // Get the title or empty string if no match
//   });
