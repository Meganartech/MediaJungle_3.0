...
class _MoviePlayerPageState extends State<MoviePlayerPage> {
  ...
  // Future<void> _fetchCastAndCrewList(List<Map<String,dynamic>> castData) async {
  //   try {
  //     List<CastMember> castList =[];
  //     for(var cast in castData){
  //       int castId = cast['id'];
  //       String castName = cast['name'];
  //       String imageUrl = await _apiService.fetchCastAndCrewImage(castId);

  //       castList.add(CastMember(id: castId, name: castName, image: imageUrl));

  //       setState(() {
  //     _castAndCrew = castList;
  //   });
  ...
  final videoDetails = await _apiService.fetchMovieDetail(id);
    setState(() {
   //   _castCrew = videoDetails.castAndCrewList; // Use the new castAndCrew list
    });
      _showControls = true;
          ...
          Container(
            ...
            child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  CastMember cast = _castAndCrew[index]; 
                  return Padding(
...
class _MoviesPlayerPageState extends State<MoviesPlayerPage> {
  ...
  //   try {
  //     final castAndCrew = await _apiService.fetchCastAndCrewList(castIds);
  //     setState(() {
  //       _castAndCrew = castAndCrew;
  //     });
  //    // return _initializeVideoPlayer(widget.videoDescriptions[_currentIndex].id);
  //   } catch (e) {
